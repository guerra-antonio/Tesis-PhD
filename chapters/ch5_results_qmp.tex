...
To assess the accuracy of the predicted quantum marginals, we use the mean fidelity, denoted $F_{\mathrm{mean}}$, built upon the squared state fidelity, i.e.\ the square of the state fidelity introduced in Section~\ref{sec:fidelity}. For each sample $\beta$ in the test set, we compute the average fidelity $F_\beta$ over all the marginals in that sample as
\begin{equation}
