...
This naturally leads to a fundamental question: what does it mean for a model to \emph{learn}? A widely accepted and operational definition, originally formulated by Mitchell and reproduced in Ref.~\cite{Goodfellow2016}, states that
\begin{quote}
...
\begin{equation}
    \label{eq:layer_1}
    a^{(1)}_i = f\qty( \boldsymbol{\omega}^{(1)}_i \cdot \mathbf{x} + b^{(1)}_i ),
    \qquad i = 1,\dots,n_1,
\end{equation}
where $\boldsymbol{\omega}^{(1)}_i$ and $b^{(1)}_i$ denote the weights and bias associated with the $i$-th neuron of the layer, respectively, and $f$ is the activation function applied element-wise. Here the superscript labels the layer index, so that $\mathbf{a}^{(1)} = (a^{(1)}_1,\dots,a^{(1)}_{n_1})$ collects the outputs of the first layer.
...
\begin{equation}
    a^{(2)}_j = g \qty( \boldsymbol{\omega}^{(2)}_j \cdot \mathbf{a}^{(1)} + b^{(2)}_j ),
\end{equation}
where $\boldsymbol{\omega}^{(2)}_j$ and $b^{(2)}_j$ are the weights and bias of the corresponding neuron in the second layer. The activation function $g$ may differ from $f$, although in practice it is common—but not mandatory—to use the same activation across multiple layers.
...
\end{equation}
where $I$ denotes the input array, $K$ the kernel, and $(i,j)$ label spatial coordinates.\footnote{Most deep learning frameworks, including the one used in this work, implement the closely related \emph{cross-correlation} operation $S(i,j)=\sum_{m,n} I(i+m,\,j+n)\,K(m,n)$, which coincides with the convolution up to a reflection of the kernel. Since the kernel entries are learned, this distinction does not affect the expressive power of the layer, and the term \emph{convolution} is retained by convention.}
...
Figure~\ref{fig:conv-connected-network} illustrates how a convolutional layer transforms an input matrix into a feature map; its spatial size depends on the padding and stride, and is reduced in the example shown, where no padding is applied, although a suitable padding can also be used to preserve it. A natural comparison between fully connected and convolutional layers can be drawn in terms of the number of internal connections, i.e.\ the number of free parameters. By restricting each neuron to interact only with a local neighborhood of the input, convolutional layers drastically reduce the number of trainable parameters. Consequently, the weights are determined by local correlations in the data, yielding a significant reduction in computational cost while preserving the ability to extract relevant features. This same inductive bias is, however, also a limitation: if the relevant structure of the data is not governed by local (near-neighbor) correlations, a model relying solely on convolutional layers cannot be expected to generalize well.

\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{images/tex-figures/conv2d.pdf}
    \caption[Two-dimensional convolution operation]{Schematic representation of a two-dimensional convolution. A $3\times3$ kernel is applied across the input array, and each output entry is obtained as the weighted sum of the corresponding local patch, producing a feature map of reduced spatial size.}
    \label{fig:conv-connected-network}
\end{figure}

% =====================================================================
\paragraph{Transposed convolutional layers}
Convolutional layers, particularly when combined with strides or pooling operations, progressively reduce the spatial resolution of the feature maps. While this contraction is desirable for tasks such as classification, many problems require the opposite operation, namely recovering or increasing the spatial resolution of a representation. Representative examples include the decoder stage of autoencoders, generative models, semantic segmentation, and super-resolution. \emph{Transposed convolutional layers} (also called fractionally-strided convolutions) provide a learnable upsampling operation that fulfils this role.
...
\begin{equation}
    \mathbf{x} \;\longmapsto\; \mathbf{C}\,\mathbf{x}
    \quad (\text{convolution}),
    \qquad
    \mathbf{z} \;\longmapsto\; \mathbf{C}^{\top}\mathbf{z}
    \quad (\text{transposed convolution}).
\end{equation}

Both operators are built from the same kernel parameters; the transposed convolution is therefore \emph{not} the inverse of the convolution, since it does not recover the original input, but rather its adjoint in terms of connectivity. This is precisely the operation that arises when backpropagating gradients through a convolutional layer: the gradient of the output with respect to the input of a convolution is obtained through a transposed convolution, and conversely.

In practice, a transposed convolution with stride $s$ can be implemented as an ordinary, unit-stride convolution applied to a modified input. Two zero-insertion operations are involved. First, the input is \emph{dilated} by inserting $s-1$ zeros between consecutive elements along each spatial dimension, which encodes the upsampling factor. Second, a zero-valued \emph{border} of width $k - p - 1$ is added around the dilated map, where $k$ is the kernel size and $p$ the padding of the corresponding direct convolution. A standard convolution with unit stride is then applied to the resulting array. Figure~\ref{fig:transconv-connected-network} shows this procedure for a $3\times3$ input and a $3\times3$ kernel: the inserted and border zeros are made explicit, and the final stride-one convolution yields a $7\times7$ output. It is worth emphasizing that these zeros are not inert; during the convolution they merely deactivate selected terms of the weighted sum, so that different output positions combine different subsets of input values and kernel weights, producing a genuine upsampling rather than a trivial zero-filling.

For a one-dimensional input of size $i$, kernel size $k$, stride $s$, and padding $p$, the spatial size of the transposed-convolution output is given by
\begin{equation}
    o = s\,(i-1) - 2p + k + a,
    \label{eq:tconv-size}
\end{equation}
where $a$, with $0 \le a < s$, is an optional \emph{output padding} introduced to resolve the ambiguity that arises because several input sizes of a direct convolution may map to the same output size. In contrast to the direct convolution, whose output is generally smaller than its input, the transposed convolution typically produces a larger output, which is the desired upsampling behavior.

Analogously to the convolutional case, the forward propagation through a transposed convolutional layer can be written as
\begin{equation}
    \mathbf{a}^{(l)} =
    \sigma^{(l)}\!\left(
    \mathbf{K}^{(l)} \,*^{\!\top}\, \mathbf{a}^{(l-1)} + \mathbf{b}^{(l)}
    \right),
\end{equation}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{images/tex-figures/transconv2d.pdf}
    \caption[Two-dimensional transposed convolution]{Schematic representation of a
    two-dimensional transposed convolution implemented as a direct convolution.
    The $3\times3$ input is dilated by inserting zeros (stride $2$), padded with a
    zero border, and convolved with a $3\times3$ kernel using unit stride,
    yielding a $7\times7$ output.}
    \label{fig:transconv-connected-network}
\end{figure}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{images/tex-figures/activation-functions.pdf}
...
\begin{figure}[ht]
    \centering
    \includegraphics[width=0.9\linewidth]{images/tex-figures/autoencoder.pdf}
    \caption[Convolutional autoencoder architecture]{Schematic representation of a convolutional autoencoder. The encoder applies convolutional layers and activations to compress the input into a low-dimensional latent representation, while the decoder applies transposed convolutional layers and activations to reconstruct the input from that latent code.}
...
To illustrate the physics-informed approach, representative examples are now considered where PINNs are employed to solve differential equations of increasing complexity.
...
Fixed (Dirichlet) boundary conditions are considered,
\begin{equation}
    u(0,t) = u(1,t) = 0,
\end{equation}
...
As a more complex example, the nonlinear Klein--Gordon equation in $(2+1)$ dimensions is considered,
\begin{equation}
