...
\begin{itemize}
    \item[(1)] Hermiticity: $\rho = \rho^{\dagger}$.
    \item[(2)] Normalization: $\Tr(\rho) = 1$.
    \item[(3)] Positivity: $\rho$ is positive semidefinite, i.e., $\bra{\psi}\rho\ket{\psi} \geq 0$ for all $\ket{\psi}$, or equivalently, all its eigenvalues satisfy $\lambda_i \geq 0$.
\end{itemize}
...
Section~\ref{sec:postulates} introduced the density operator formalism. This section discusses several structural properties and derived quantities that follow directly from the definition of the density operator and will be used throughout this work.
...
\begin{equation}
    \gamma(\rho) = \Tr(\rho^2).
\end{equation}
This quantity satisfies $\frac{1}{d} \le \gamma(\rho) \leq 1$ for a $d$-dimensional system, with $\gamma(\rho) = 1$ corresponding to pure states and the lower bound $\gamma(\rho)=1/d$ attained by the maximally mixed state $\rho = \id/d$; smaller values indicate increasing degrees of mixedness. In particular, if $\gamma(\rho) < 1$, the density operator cannot be written in the form $\rho = \ket{\psi}\bra{\psi}$ for some state vector $\ket{\psi}$.
...
From a formal perspective, entanglement is naturally defined within the density operator formalism. Consider a bipartite quantum system composed of two subsystems $A$ and $B$, described by a joint density operator $\rho_{AB}$ acting on the composite Hilbert space $\mathcal{H}_A \otimes \mathcal{H}_B$. The state $\rho_{AB}$ is said to be \emph{separable} if it can be expressed as a convex combination of product states, namely,
\begin{equation}
    \label{eq:separable}
...
\label{sec:quantum-dynamics}

\paragraph{Equation of motion for the density operator.}
...
Throughout this work, the Schrödinger picture is adopted.
...
Differentiating this expression with respect to time, using Eq.~\eqref{eq:U_evolution}, and exploiting the Hermiticity of the Hamiltonian, $H(t)=H^{\dagger}(t)$, one obtains
\begin{align}
    \dv{t} \rho(t)
     & = \dv{t} U(t,t_0) \,\rho_0\,U^{\dagger}(t,t_0)
    + U(t,t_0)\,\rho_0\,\dv{t} U^{\dagger}(t,t_0) \nonumber \\
     & = -i H(t)\rho(t) + i \rho(t)H(t) \nonumber           \\
     & = -i\,[H(t),\rho(t)], \label{eq:von_neumann}
\end{align}
...
\begin{equation}
    ...
    \label{eq:time_ordered_exponential}
\end{equation}
...
\begin{align}
    ...
    \Big),
\end{align}
while higher-order terms involve increasingly nested commutators of the Hamiltonian evaluated at different times. The Magnus series converges, and the exponential representation is therefore valid, provided that $\int_{t_0}^{t} \norm{H(\tau)}_2 \, \mathrm{d}\tau < \pi$, where $\norm{\cdot}_2$ denotes the spectral (operator) norm~\cite{blanes2009}; this convergence condition underlies the normalization of the Hamiltonian introduced in the applications of Chapter~\ref{ch:results-pinn-unitaries}.
...
It is useful to introduce a correspondence between vectors in $\mathbb{R}^3$ and the space of traceless Hermitian $2 \times 2$ matrices. This correspondence is naturally established through the Pauli matrices, which span the traceless Hermitian $2\times 2$ matrices, i.e.\ the generators of $SU(2)$ (the elements of $\mathfrak{su}(2)$ up to a factor of $i$). Within this framework, any single-qubit density operator can be parametrized in terms of a real three-dimensional vector, known as the Bloch vector.

Explicitly, a one-qubit density operator can be written as
\begin{equation}
    \rho = \frac{1}{2}\qty(\id + \va{r} \vdot \vb*{\sigma}),
\end{equation}
...
\begin{equation}
    r_i = \mathrm{Tr}(\rho\, \sigma_i), \qquad i \in \{1,2,3\},
\end{equation}
...
The fidelity between two quantum states $\rho$ and $\sigma$ is defined as \cite{Jozsa1994}
\begin{equation}
    \label{eq:fidelity}
    F(\rho,\sigma)
    =
...
\begin{itemize}
    ...
    \item \textbf{Joint concavity of the square-root fidelity:}
          For any sets $\{\rho_i\}$, $\{\sigma_i\}$ and probabilities $\{p_i\}$,
          \begin{equation}
              \sqrt{F}\qty(\sum_i p_i \rho_i, \sum_i p_i \sigma_i ) \geq \sum_i p_i \sqrt{F(\rho_i,\sigma_i)}.
          \end{equation}
\end{itemize}
...
Unitary channels correspond to pure Choi states, which can be written as $J_U = \ket{J_U}\bra{J_U}$ and $J_V = \ket{J_V}\bra{J_V}$. For the comparison of unitary channels, the \emph{gate fidelity} is adopted, defined simply as the fidelity of Eq.~\eqref{eq:fidelity} evaluated on these pure Choi states,
\begin{equation}
    F(J_U, J_V) = \qty| \braket{J_U}{J_V} |^2 .
\end{equation}
...
\begin{equation}
    ...
    F(J_U, J_V)
    =
    \qty| \bra{\Phi} (\id \otimes U^\dagger V) \ket{\Phi} |^2
    =
    \qty| \frac{1}{d}\, \Tr(U^\dagger V) |^2 .
\end{equation}

This quantity, which corresponds to the transition probability between the pure Choi states associated with $U$ and $V$, is commonly referred to as the \emph{gate fidelity}. In particular, if $U = V$, then $\Tr(U^\dagger V) = d$, yielding $F(J_U, J_V) = 1$, which corresponds to maximal fidelity. The fidelity between Choi matrices therefore provides a consistent and operationally meaningful measure for comparing unitary quantum operations.
